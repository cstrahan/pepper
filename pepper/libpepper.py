...
class Pepper:
    ...
    def req_get(self, path: str) -> Optional[Dict[str, JSONType]]:
        """
    ...
    def req(self, path: str, data: Optional[JSONType] = None) -> Dict[str, JSONType]:
        """
        ...
        """
        if (
            isinstance(data, dict) and data.get("eauth") == "kerberos"
        ...
        if path != "/run" and self.auth and "token" in self.auth and self.auth["token"]:
            req.add_header("X-Auth-Token", str(self.auth["token"]))
    ...
    def req_requests(
        self, path: str, data: Optional[JSONType] = None
    ) -> Dict[str, JSONType]:
        """
        ...
        """
        import requests
        from requests_gssapi import HTTPSPNEGOAuth, OPTIONAL  # type: ignore
    ...
    def low(self, lowstate: JSONType, path: str = "/") -> Dict[str, JSONType]:
        """
    ...
    def local(
        ...
        tgt: str,
        fun: str,
        arg: Optional[List[JSONType]] = None,
        kwarg: Optional[Dict[str, JSONType]] = None,
        expr_form: str = "glob",
    ...
    def lookup_jid(self, jid: Union[str, int]) -> Dict[str, JSONType]:
        """
        Get job results
    ...
    def runner(
        self, fun: str, arg: Optional[List[JSONType]] = None, **kwargs: JSONType
    ) -> Dict[str, JSONType]:
        """
    ...
    def _send_auth(self, path: str, **kwargs: JSONType) -> Dict[str, JSONType]:
    ...
    def login(
        ...
        **kwargs: Any,
        ...
        self.auth = self._send_auth("/login", **kwargs).get("return", [{}])[0]  # type: ignore
        return self.auth

    def token(self, **kwargs: JSONType) -> Dict[str, JSONType]:
        """
